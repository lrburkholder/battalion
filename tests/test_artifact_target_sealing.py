...
from battalion.application import (
    ArtifactTargetHandoffRejected, CreateAdmittedRun, SealArchitectTargetHandoff,
    create_admitted_run, seal_architect_target_handoff,
)
...
from battalion.graph import _make_architect_node, run_ticket, resume_ticket
...
def handoff_project(tmp_path, request):
    ...
    config = BattalionConfig(
        ...
        manual_checkpoints=["driver"] if getattr(request, "param", True) else [],

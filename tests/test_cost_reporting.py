...
from battalion.state.models import Budget, LLMCallCost, NodeExecution, RunState, RunStatus
    ...
    assert summary == {
        ...
        "phases": [
            {"phase": "architect", "role": "architect", "calls": 1,
             "input_tokens": 10, "output_tokens": 4, "cost_usd": 0.002},
            {"phase": "driver_green", "role": "driver", "calls": 1,
             "input_tokens": 20, "output_tokens": 8, "cost_usd": 0.003},
        ],
    }
...
def test_status_costs_queries_saved_run_without_changing_turn_budget(tmp_path, monkeypatch):
    ...
    assert "architect: 1 call(s), 10 in / 4 out, $0.002000" in result.output

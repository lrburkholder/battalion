"""Validate and render the public status projection from GitHub Issues.
...
def _milestone_rows(tickets: Sequence[Ticket]) -> list[str]:
    """Summarize each canonical milestone before linking to issue detail."""

    groups: dict[str, list[Ticket]] = {}
    for ticket in tickets:
        groups.setdefault(ticket.milestone or "Unassigned", []).append(ticket)
    rows: list[str] = []
    for milestone, members in sorted(groups.items()):
        counts = {
            status: sum(ticket.status == status for ticket in members)
            for status in ("done", "in-progress", "in-review", "blocked", "not-started", "cancelled")
        }
        active = counts["in-progress"] + counts["in-review"] + counts["blocked"]
        rows.append(
            f"| {milestone} | {len(members)} | {counts['done']} | {active} | "
            f"{counts['not-started']} | {counts['cancelled']} |"
        )
    return rows
...
def render_delivery_section(tickets: Sequence[Ticket]) -> str:
    ...
    lines = [
        ...
        "| --- | ---: | ---: | ---: | ---: | ---: |",
        *_milestone_rows(tickets), "", "### Active work", "",
...
def main() -> None:
    ...
    if args.validate:
        load_tickets(fetch_github_issues())
        print("canonical GitHub Issue corpus is valid")
        return
    stale = sync_documents()
    if stale:
